...
class roipoly:
    ...
    def __motion_notify_callback(self, event):
        if event.inaxes:
            ...
            if (
                event.button == None or event.button == 1
            ) and self.line != None:  # Move line around
                self.line.set_data(
            ...
            if (
                ...
                if self.line == None:  # if there is no line, create a line
                    self.line = plt.Line2D(
                        [x, x], [y, y], marker="o", color=self.roicolor
                    )
                ...
                (event.button == 1 and event.dblclick == True)
                or (event.button == 3 and event.dblclick == False)
            ) and self.line != None:  # close the loop and disconnect
